...
const responseSchema = {
  ...
  properties: {
    accountName: {
      type: Type.STRING,
      description: "The primary account holder's name or company name found in the statement. It should be 'Warba Medical Polyclinic'.",
    },
    accountNumber: {
      type: Type.STRING,
      description: "The bank account number found in the statement. This can be a long numeric string or a shorter alphanumeric identifier. Extract whichever is present.",
    },
    transactions: {
      type: Type.ARRAY,
...
export async function extractTransactionsFromText(text: string): Promise<ExtractedData> {
  ...
  const prompt = `
    ...
    1. Identify the primary account holder's name (It should be "Warba Medical Polyclinic").
    2. Identify the bank Account Number for the statement.
  ...
  try {
    ...
    try {
        ...
        if (rawText.toLowerCase() === 'undefined') {
             throw new Error("The AI failed to extract data.");
        }
